...
This module is portable Hermes Helmet setup + diagnostics + a protocol-faithful
...
Hermes Helmet starts and operates when FAVA Trails is declined or unavailable.
"""
...
# Inspected FAVA baseline for Hermes Helmet H7 (0.7.0 / MCP 2.2).
...
def render_data_repo_config_yaml(template: FavaCompanyTemplate) -> str:
    ...
    env_name = validate_env_var_name(template.openrouter_api_key_env, field="openrouter_api_key_env")
    return (
        f"# Generated by helmet fava setup — no secrets.\n"
...
class FakeFavaEngine:
    """In-memory FAVA lifecycle model for hermetic Hermes Helmet verification.
    ...
    *,
    ...
    try:
        ...
        if not (home / ".jj").exists():
            ...
            subprocess.run(
                [jj_bin, "config", "set", "--repo", "user.name", "Hermes Helmet FAVA Demo"],
                cwd=str(home),
                check=True,
                capture_output=True,
                text=True,
        ...
        return DoctorReport(
            ...
            findings=(
                DoctorFinding(
                    code="fava-declined",
                    ok=True,
                    message="FAVA Trails integration declined; Hermes Helmet operates without it",
                ),
            ),
        )
...
def setup_messages(template: FavaCompanyTemplate) -> list[str]:
    return [
        ...
        "Run helmet fava lifecycle-demo for the hermetic governed example.",
        "Hermes Helmet starts and operates when FAVA Trails is declined or unavailable.",
    ]
