...
`helmet setup`; do not reproduce setup logic in agent prose.
...
2. Ask the operator to create a separate fine-grained worker PAT with the
   ...
   selected repositories. Extra public visibility is not a Hermes Helmet failure; extra
   private visibility requires an explicit `worker_access_scope: broader`
   adopter choice (`selected` is the default).
3. Run `helmet setup --answers /path/to/answers.json`. The command uses
...
7. Run `helmet doctor --config ~/.hermes-helmet/policy.json --home ~ --live`
   ...
   group/other writable. Every existing component of Hermes Helmet state/generated/
